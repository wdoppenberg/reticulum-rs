...
/// Encode a `PrivateIdentity` to 64 raw bytes (no alloc, no hex).
fn identity_to_bytes(identity: &PrivateIdentity) -> [u8; IDENTITY_BYTES] {
    // Re-derive the raw bytes via the hex round-trip string, using only the
    // &str → parse path which is available without alloc on the read side.
    //
    // Because `to_hex_string` requires `alloc`, we extract the bytes directly
    // from the key references exposed through the identity API.
    //
    // PrivateIdentity exposes:
    //   - `.private_key` (x25519_dalek::StaticSecret) via `.private_key.as_bytes()` if we
    //     can access the field, or indirectly through the exchange method.
    //
    // Since the struct fields are not pub, we go through `new_from_hex_string`
    // in reverse: we build a hex string on the stack using heapless::String.
    let mut raw = [0u8; IDENTITY_BYTES];

    // Use the hex round-trip: build a fixed-size hex string on the stack via
    // the heapless String (131 chars + null — we need 128 hex chars + 1 for
    // safe margin).
    let hex_len = IDENTITY_BYTES * 2; // 128
    let mut hex_buf = [0u8; 128];

    // We cannot call to_hex_string() without alloc, but we can use the
    // new_from_hex_string / sign path to reconstruct bytes via a round-trip
    // through heapless storage.
    //
    // Alternative: directly call private field via unsafe? No.
    //
    // The cleanest no_alloc solution is to extend reticulum-core with a
    // `to_bytes()` method. Until then we use the heapless String approach:
    // build the hex string without allocation, using the `write!` macro into
    // a stack `[u8; 128]` buffer.
    let mut pos = 0usize;

    // We extract bytes by constructing a temporary hex representation in a
    // fixed stack buffer. The identity's private key bytes are accessible via
    // the `exchange` method's DH scalar — but that's not exposed directly.
    //
    // The correct approach is to call the core crate's serialisation in a
    // no_alloc compatible way. We work around by using `new_from_hex_string`
    // in reverse with the `format_hex` helper below.
    //
    // NOTE: This requires reticulum-core to expose `to_raw_bytes()` for a
    // proper no_alloc path. For now we call through the Sign key's `as_bytes`
    // which IS accessible, and reconstruct the private key bytes from the
    // Diffie-Hellman scalar via the `x25519_dalek` repr.
    //
    // ──────────────────────────────────────────────────────────────────────────
    // Practical solution: use `new_from_hex_string` round-trip with a
    // heapless::String and the `alloc`-gated `to_hex_string`.
    //
    // Since `storage` requires `alloc` to be enabled in reticulum-core for
    // hex conversion, we instead store raw bytes by encoding the key material
    // through the hex API, enabled with the `alloc` feature.
    //
    // If no alloc: fall back to `new_from_name`-based reconstruction which is
    // deterministic — but that defeats the purpose of random identity.
    //
    // The real fix is a `PrivateIdentity::to_raw_bytes() -> [u8; 64]` in core.
    // We keep this note here as a design debt marker and require `alloc` for
    // the storage feature.
    let _ = (raw, hex_buf, pos, hex_len); // suppress unused warnings
    unimplemented!(
        "identity_to_bytes requires reticulum-core to expose PrivateIdentity::to_raw_bytes(); \
         see storage.rs design note"
    )
}
